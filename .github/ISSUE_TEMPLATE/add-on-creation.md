---
name: Add-on Creation
about: Describe a new addon with the features that must be implemented
title: ''
labels: ''
assignees: ''

---

# Addon

#### Observação Prévia: Toda a codebase DEVE ser documentada e escrita de preferência em Typescript. 

# Nome Do Addon

Exemplo: Wallet System

## Description

Explique o que o addon se propõe. E o que seria o addon de uma forma geral

### Itens e Blocos
* Quais itens e blocos vão ter no addon
* Quais as formas de se obter os addon
* Se tiver receitas, descreva todas as receitas

### Entidades
* Quais entidades vão ter no addon.
* O que cada entidade faz, e sistemas que são relacionados a ela. 

### Sistemas mais complexos
* Abra uma subseção com 4 # e use ela pra descrever todas as coisas que o sistema tem de ter
* Dê também(preferencialmente) uma ideia breve de como o sistema deve ser implementado internamente
* Como o sistema é usado pelo user.
* Explique regras importantes do projeto e edge cases

### Gameplay
* Diga como o player é idealizado pra jogar o addon
* Se houver progressão, escreva como o player deve fazer essa progressão e se sistemas do addon houverem algum tipo de ligação, descreva como eles se ligam

### 🖥 Interfaces (UI)

* Se houver interfaces, defina como a interface deve ser feita
* Dê imagens de referência pra como a UI deve ficar no final
* Exemplifique o que cada coisa da UI faz

### Assets

Defina que tipos de assets serão necessários, comos:
* Texturas
* Sons
* Modelos
* Animações

### Guidebook

* Conteúdo que precisa entrar no guia.
* Explicação da feature
* Como usar
* Exemplos


### Marketplace Requirements

Defina coisas que são necessárias pra a submissão no market place:
* Icons finalizados
* UI finalizada
* Guidebook atualizado
* Screenshots

---
